...
import DLSumGagueChart from "./DLSumGagueChart";

const TopDLThroughput: React.FC = () => {
  // useQuery로 실시간 데이터 패칭
  const { data: cellData } = useQuery({
    //useQuery가 반환하는 객체 중 data를 cellData라는 이름으로 할당 (서버에서 데이터를 받아와 컴포넌트에서 사용 가능)
    ...
    <div className={styles.container}>
      ...
      <section className={styles.item}>
        ...
        <Charts
          ...
          colors={chartColors}
          dataKey="UL_rate"
          title="Power (Cells)" // title="DL_rate" -> Cells
          name="Cell_ID"
          yLabel="kW"
        />
